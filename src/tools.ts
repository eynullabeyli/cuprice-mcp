...
export const DEFAULT_CUPRICE_BASE = "https://app.cuprice.io";
...
async function cupriceAPI(path: string): Promise<unknown> {
  const res = await fetch(`${getCupriceBase()}${path}`, {
...
function getCupriceBase(): string {
  return process.env.CUPRICE_BASE || DEFAULT_CUPRICE_BASE;
}

type Framework = "html" | "nextjs" | "react" | "vue";
type Widget = "pricing" | "billing";

export function buildEmbedCode(
  shareId: string,
  framework: Framework = "html",
  widget: Widget = "pricing",
  base: string = getCupriceBase(),
): string {
...
export function createServer(): McpServer {
  ...
  server.registerTool("get-embed-code", {
    ...
    return { content: [{ type: "text", text: buildEmbedCode(shareId, framework, widget) }] };
  });
  ...
  server.registerTool("get-css-classes", {
    ...
    return jsonResult(getCssClasses());
  });
